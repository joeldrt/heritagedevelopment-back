...
from hd_app.api.usuario.servicios import crear_usuario, obtener_usuarios_todos, editar_usuario, borrar_usuario, \
    obtener_usuarios_todos_root
from hd_app.api.usuario.serializadores import usuario, usuario_alta, usuario_editar
...
ns = api.namespace('usuarios', description='Servicios para administrar a los usuarios')
...
class User(Resource):
    ...
    def post(self):
        ...
        if UserModel.find_by_login(data.get('login')):
            abort(400, 'Ya existe un usuario con ese nombre de usuario')
        ...
        for authority_str in data.get('authorities'):
            if not current_user.has_authority(authority_str):
                abort(401, 'No se pueden asignar autoridades para el usuario con las no cuentas')

        try:
            user = crear_usuario(login=data.get('login'), password=data.get('password'),
                                 first_name=data.get('firstName'), last_name=data.get('lastName'),
                                 email=data.get('email'), authorities=data.get('authorities'))
            return user.to_json()
    ...
    def get(self):
        ...
        if current_user.has_authority(AUTHORITY_ROOT):
            usuarios = obtener_usuarios_todos_root()
        else:
            usuarios = obtener_usuarios_todos()
        return usuarios
...
class UserActions(Resource):
    ...
    def delete(self, usuario_id):
        ...
        if not current_user.has_authority(AUTHORITY_ROOT) and \
                UserModel.find_by_id(usuario_id).login == ROOT_OBJECT.get('login'):
...
class ChangeUserPassword(Resource):
    ...
    def put(self, usuario_id):
        ...
        if not current_user.has_authority(AUTHORITY_ROOT) and user.login == ROOT_OBJECT.get('login'):
